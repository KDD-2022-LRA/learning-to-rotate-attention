...
from layers.DecompAttention import QuaternionAttention, LearningToRotateAttentionLayer
from layers.Deformer_EncDec import Encoder, Decoder, EncoderLayer, DecoderLayer, my_Layernorm, series_decomp
import math
import numpy as np
...
class Model(nn.Module):
            ...
            [
                EncoderLayer(
                    LearningToRotateAttentionLayer(
                        ...
                        self.seq_len, self.seq_len, configs.d_model, configs.n_heads, moving_avg=configs.moving_avg, period_type=configs.period_type, n_periods=configs.n_periods, is_trend_refinement=True),
                    configs.d_model,
                    d_ff = configs.d_ff,
            ...
            [
                DecoderLayer(
                    LearningToRotateAttentionLayer(
                        ...
                        self.label_len + self.pred_len, self.label_len + self.pred_len, configs.d_model, configs.n_heads, moving_avg=configs.moving_avg, period_type=configs.period_type, n_periods=configs.n_periods, is_trend_refinement=True),
                    LearningToRotateAttentionLayer(
                        Attention(self.label_len + self.pred_len, self.seq_len, False, attention_dropout=configs.dropout,
                                        output_attention=False),
                        self.label_len + self.pred_len, self.seq_len, configs.d_model, configs.n_heads, moving_avg=configs.moving_avg, period_type=configs.period_type, n_periods=configs.n_periods, is_trend_refinement=False),
                    configs.d_model,
                    d_ff = configs.d_ff,
                    dropout=configs.dropout,
                    activation=configs.activation,
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec,
        ...
        # decoder input
        seasonal_init = torch.cat([x_enc[:, -self.label_len:, :], mean], dim=1)
        # enc
        enc_out = self.enc_embedding(x_enc, x_mark_enc)
        enc_out, attns, omegas_penalties_enc, thetas_penalties_enc = self.encoder(enc_out, attn_mask=enc_self_mask)
        # dec
        dec_out = self.dec_embedding(seasonal_init, x_mark_dec)
        dec_out, omegas_penalties_dec, thetas_penalties_dec = self.decoder(dec_out, enc_out, x_mask=dec_self_mask, cross_mask=dec_enc_mask)
